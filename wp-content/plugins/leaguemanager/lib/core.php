...
class LeagueManager
{
	...
	function getImagePath( $file = false )
	{
		...
		if ( $file ) 
			return ABSPATH . substr($file,strlen($_SERVER['HTTP_HOST'])+8, strlen($file));
		 else 
	...
	{
		return ABSPATH . dirname(substr($file,strlen($_SERVER['HTTP_HOST'])+8, strlen($file))) . '/thumb_' . basename($file);
	}
